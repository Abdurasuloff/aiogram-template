...
class ThrottlingMiddleware(BaseMiddleware):
    def __init__(self, storage: SQLStorage, limit=.6, key_prefix='antiflood_'):
        self.rate_limit = limit
        self.prefix = key_prefix
        self.throttle_manager = ThrottleManager(storage=storage)
    ...
    async def event_throttled(self, event: Message, throttled: Throttled):
        ...
        if throttled.exceeded_count <= 2:
            await event.answer("So'rovlar juda ko'p!")
...
class ThrottleManager:
    ...
    async def throttle(self, key: str, rate: float, user_id: int, chat_id: int):
        now = time.time()
        storage_key = StorageKey(bot_id=12, chat_id=chat_id, user_id=user_id)
        ...
        await self.storage.update_data(storage_key, data)
